...
if img is not None:
    ...
    if st.sidebar.checkbox("Mostrar vista 3D interactiva"):
        with st.spinner("Generando vista 3D... esto puede tardar varios segundos"):
            # Reducir resolución para optimizar visualización
            factor = 4  # Puedes ajustar este valor
            img_small = img[::factor, ::factor, ::factor]
            ...
            fig3d.update_layout(
                scene=dict(
                    xaxis_title="Eje Z (Axial)",
                    yaxis_title="Eje Y (Coronal)",
                    zaxis_title="Eje X (Sagital)",
                    aspectmode='data'
                ),
